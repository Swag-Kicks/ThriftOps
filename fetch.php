...
if(isset($_POST['cond']))
{
    ...
    if($cond=="all")
    {
        ...
        if($_POST['vendor'] != '')
        {
            ...
            if($_POST['lot'] != '')
            {
                $query .= ' WHERE Vendor_ID="'.$_POST['vendor'].'" AND Batch_ID ="'.$_POST['lot'].'" AND Status in ("active","draft","archieve") AND Warehouse_ID="'.$_POST['location'].'"';
            }
            //status
        ...
        else if($_POST['lot'] != '')
        {
            if($_POST['status'] != '')
            {
                ...
                $query .= ' WHERE Batch_ID ="'.$_POST['lot'].'" AND Status = "'.$_POST['status'].'" AND Warehouse_ID="'.$_POST['location'].'"';
            }
            else
            {
                $query .= ' WHERE Batch_ID ="'.$_POST['lot'].'" AND Status in ("active","draft","archieve") AND Warehouse_ID="'.$_POST['location'].'"';
            }
        }
        //status
        else if($_POST['status'] != '')
    ...
    if($cond=="activein")
    {
        ...
        if($_POST['vendor'] != '')
        {
            ...
            if($_POST['lot'] != '')
            {
                $query .= ' WHERE Vendor_ID="'.$_POST['vendor'].'" AND Batch_ID ="'.$_POST['lot'].'" AND Status ="active" AND Quantity >= "1" AND Warehouse_ID="'.$_POST['location'].'"';
            }
            //status
            else if($_POST['status'] != '')
            {
        ...
        else if($_POST['lot'] != '')
        {
            if($_POST['status'] != '')
            {
                ...
                $query .= ' WHERE Batch_ID ="'.$_POST['lot'].'" AND Status = "'.$_POST['status'].'" AND Quantity >= "1" AND  Warehouse_ID="'.$_POST['location'].'"';
            }
            else
            {
                $query .= ' WHERE Batch_ID ="'.$_POST['lot'].'" AND Status ="active" AND Quantity >= "1" AND Warehouse_ID="'.$_POST['location'].'"';
            }
        }
        //status
        else if($_POST['status'] != '')
    ...
    if($cond=="draftin")
    {
        ...
        if($_POST['vendor'] != '')
        {
            ...
            if($_POST['lot'] != '')
            {
                $query .= ' WHERE Vendor_ID="'.$_POST['vendor'].'" AND Batch_ID ="'.$_POST['lot'].'" AND Status="draft" AND Quantity >= "1" AND Warehouse_ID="'.$_POST['location'].'"';
            }
            //status
            else if($_POST['status'] != '')
            {
        ...
        else if($_POST['lot'] != '')
        {
            ...
            {
                $query .= ' WHERE Batch_ID ="'.$_POST['lot'].'" AND Status="draft" AND Quantity >= "1" AND Warehouse_ID="'.$_POST['location'].'"';
            }
        }
        //status
        else if($_POST['status'] != '')
    ...
    if($cond=="activeout")
    {
        ...
        if($_POST['vendor'] != '')
        {
            ...
            if($_POST['lot'] != '')
            {
                $query .= ' WHERE Vendor_ID="'.$_POST['vendor'].'" AND Batch_ID ="'.$_POST['lot'].'" AND Status ="active" AND Quantity = "0" AND Warehouse_ID="'.$_POST['location'].'"';
            }
            //status
            else if($_POST['status'] != '')
            {
        ...
        else if($_POST['lot'] != '')
        {
            if($_POST['status'] != '')
            {
                ...
                $query .= ' WHERE Batch_ID ="'.$_POST['lot'].'" AND Status = "'.$_POST['status'].'" AND Quantity = "0"  AND Warehouse_ID="'.$_POST['location'].'"';
            }
            else
            {
                $query .= ' WHERE Batch_ID ="'.$_POST['lot'].'" AND Status ="active" AND Quantity = "0" AND Warehouse_ID="'.$_POST['location'].'"';
            }
        }
        //status
        else if($_POST['status'] != '')
     ...
     if($cond=="draftout")
    {
        ...
        if($_POST['vendor'] != '')
        {
            ...
            if($_POST['lot'] != '')
            {
                $query .= ' WHERE Vendor_ID="'.$_POST['vendor'].'" AND Batch_ID ="'.$_POST['lot'].'" AND Status ="draft" AND Quantity = "0"  AND Warehouse_ID="'.$_POST['location'].'"';
            }
            //status
            else if($_POST['status'] != '')
            {
        ...
        else if($_POST['lot'] != '')
        {
            ...
            {
                $query .= ' WHERE Batch_ID ="'.$_POST['lot'].'" AND Status ="draft" AND Quantity = "0" AND Warehouse_ID="'.$_POST['location'].'"';
            }
        }
        //status
        else if($_POST['status'] != '')
...
while($row = mysqli_fetch_array($result))
{
    ...
    //lot
    $records2 = mysqli_query($mysql,"SELECT Number AS nam FROM LOT WHERE id='".$row["Batch_ID"]."'"); 
    ...
    <th>
      ...
      <td class="text-left"><a href="https://backup.thriftops.com/Upload/Edit.php?SKU='.$row["SKU"].'"  >'.$row["SKU"].'</a></td>
