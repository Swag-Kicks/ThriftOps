...
if(isset($_POST['cond']))
{
    ...
    if($cond=="Pick")
    {
        if(!empty($_POST['from']))
        {
             if(!empty($_POST['to']))
             {
                 $query .= ' WHERE Status LIKE "%Picked%" AND Date Between "'.$_POST['from'].'" AND "'.$_POST['to'].'"';
             }
        }
        else if(!empty($_POST['ordernum']))
        {
            $ord=$_POST['ordernum'];
            if(is_numeric($ord))
            {
                $query .= ' WHERE Order_Number LIKE "%#'.str_replace(' ', '%', $_POST['ordernum']).'%" AND Status in ("Picked","Not_Received")';
            }
            else
            {
               $query .= ' WHERE Order_Number LIKE "%'.str_replace(' ', '%', $_POST['ordernum']).'%" AND Status in  ("Picked","Not_Received")';
        ...
        {
          $query .= ' WHERE SKU LIKE "%'.str_replace(' ', '%', $_POST['items']).'%" AND Status in  ("Picked","Not_Received")';
        }
        else
        {
    ...
    if($cond=="Picked")
    {
        if(!empty($_POST['from']))
        {
             if(!empty($_POST['to']))
             {
                 $query .= ' WHERE Status="Received" AND Date Between "'.$_POST['from'].'" AND "'.$_POST['to'].'"';
             }
        }
          else if(!empty($_POST['ordernum']))
        {
       ...
       if(!empty($_POST['from']))
        {
             if(!empty($_POST['to']))
             {
                 $query .= ' WHERE Status="Not_Received" AND Date Between "'.$_POST['from'].'" AND "'.$_POST['to'].'"';
             }
        }
         else if(!empty($_POST['ordernum']))
        {
       ...
       if(!empty($_POST['from']))
        {
             if(!empty($_POST['to']))
             {
                 $query .= ' WHERE Status LIKE "%Packed%" AND Date Between "'.$_POST['from'].'" AND "'.$_POST['to'].'"';
             }
        }
          else if(!empty($_POST['ordernum']))
        {
...
while($row = mysqli_fetch_array($result))
{
    ...
    if($_POST['cond']=='Pack'|| $_POST['cond']=='Picked' || $_POST['cond']=='Not')
    {
        
    }
    if($_POST['cond']=='Pick')
    {
        $check='<input type="checkbox" name="check[]" value="'.$row["id"].'">';
        $Buttons= '<div class="align-middle text-center">
                                        <div class="btn-group align-top" style="margin-top: 5px;">
                                            <input ord="'.$row["Order_ID"].'" type="button" name="pick" value="Received" id="'. $row["SKU"].'" class="btn btn-md btn-primary pick_data" />
                                          </div>
                                          <div class="btn-group align-top" style="margin-top: 5px;">
                                           <input ord="'.$row["Order_ID"].'" type="button" name="not" value="Not Recevied" id="'. $row["SKU"].'" class="btn btn-md btn-secondry not_data" />
                                          </div>
