...
if(!empty($_POST['from']))
{
     if(!empty($_POST['to']))
     {
        ...
        $query .= " WHERE Order.Status = 'Dispatched' and Logs.DateTime between '$from 00:00:00' AND '$to 23:59:59'";
     }
}
else
{
    $query .= " WHERE Order.Status = 'Dispatched' and  MONTH(Logs.DateTime) = MONTH(now()) and YEAR(Logs.DateTime) = YEAR(now()) ";
}
...
if($_POST['sort'])
{
    ...
    if($sort=='ASC')
    {
        $query .= ' GROUP BY Logs.DateTime ASC';
    }
    if($sort=='DESC')
    {
        $query .= ' GROUP BY Logs.DateTime DESC';
    }
}
else
{
...
{
    $sku=$row['SKU'];
    $query1="SELECT Vendor_ID, Cndition,Category_ID, Cost, Price FROM `addition` Where SKU='$sku'";
