...
$sql4="SELECT * FROM  `racks` Where SKU ='$id' LIMIT 1";
$result4 = mysqli_query($mysql, $sql4);
$row4 =mysqli_fetch_array($result4);
$rack_pos=$row4['name'];
$warehouse_id = $row4['Warehouse_ID'];
if ($warehouse_id != 0){
  $sql5="SELECT * FROM  `Warehouse` Where Warehouse_ID = '$warehouse_id' LIMIT 1";
  $result5 = mysqli_query($mysql, $sql5);
  $row5 =mysqli_fetch_array($result5);
  $warehouse_location=$row5['Location'];
}
else{
   $warehouse_location = 'a warehouse';
}

$result2= mysqli_query($mysql,"Select * from `Logs` Where Type='Product' AND Type_ID='$shopifyid'");
// $row_cnt = mysqli_num_rows($result_order_logs);
// // $row_order_logs =mysqli_fetch_array($result_order_logs);
// if ($row_cnt == 0){
//    $result_orders= mysqli_query($mysql,"Select * from `Order` Where SKU='$id'");
//    $row_orders = mysqli_fetch_array($result_orders);
//    foreach($row_orders as $value)
//    {
//       $result_id_logs = mysqli_query($mysql,"Select * from `Logs` Where Type ='Order' AND Type_ID='$value'");
//       $row_id_logs =mysqli_fetch_array($result_id_logs);
  ...
  <div class="col-md-12 m-b-30">
    <ul class="timelineleft pb-5">
        ...
        while($row2 =mysqli_fetch_array($result2))
        {
            ...
            $status == 'Upload'? $status = 'uploaded':  $status;
            $status == 'Added'? $status = 'added':  $status;
            $status == 'draft'? $status = 'drafted':  $status;
            $status == 'active'? $status = 'activated':  $status;
            $status == 'Edited from thriftops'? $status = 'edited':  $status;

            if($status == 'Adjust Quantity')
            {
               echo "<li>
                  <i class='fa fa-circle '></i>
                  <div class='timelineleft-item'>
                     <span class='time'><i class='fa fa-clock-o text-danger'>&nbsp;</i>$datetime</span>
                     <h3 class='timelineleft-header no-border'><span class='text-muted'>Item quantity has been adjusted by $name</span></h3>
                  </div>
               </li>";
            }
            else{
               ...
               "<li>
                  ...
                  <div class='timelineleft-item'>
                     <span class='time'><i class='fa fa-clock-o text-danger'>&nbsp;</i>$datetime</span>
                     <h3 class='timelineleft-header no-border'><span class='text-muted'>Item  has been $status by $name</span></h3>
                  </div>
               </li>";
            }

           
            // display rack and warehouse where stored
            if ($rack_pos != ''){ 
               ...
               echo 
                  "<li>
                     ...
                     <div class='timelineleft-item'>
                        <span class='time'><i class='fa fa-clock-o text-danger'>&nbsp;</i>$datetime</span>
                        <h3 class='timelineleft-header no-border'><span class='text-muted'>Item has been auto-allocated at $rack_pos in $warehouse_location</span></h3>
                     </div>
                  </li>";
         ...
         //            <div class='timelineleft-item'>
         //                <span class='time'><i class='fa fa-clock-o text-danger'>&nbsp;</i>$datetime</span>
         //                <h3 class='timelineleft-header no-border'><span class='text-muted'>Item has been uploaded by $name</span></h3>
         //            </div>
         //        </li>";
         //    }
         //    //added
         ...
         //            <div class='timelineleft-item'>
         //                <span class='time'><i class='fa fa-clock-o text-danger'>&nbsp;</i>$datetime</span>
         //                <h3 class='timelineleft-header no-border'><span class='text-muted'>Item has been added by $name</span></h3>
         //            </div>
         //        </li>";
         //    }
         //    //draft
         ...
         //            <div class='timelineleft-item'>
         //                <span class='time'><i class='fa fa-clock-o text-danger'>&nbsp;</i>$datetime</span>
         //                <h3 class='timelineleft-header no-border'><span class='text-muted'>Item has been drafted by $name</span></h3>
         //            </div>
         //        </li>";
         //    }
            
         //    ///adjust quantity
         //    else if($status == 'Adjust Quantity')
         //    {
         //       echo "<li>
         //          <i class='fa fa-circle '></i>
         //          <div class='timelineleft-item'>
         ...
         //            <div class='timelineleft-item'>
         //                <span class='time'><i class='fa fa-clock-o text-danger'>&nbsp;</i>$datetime</span>
         //                <h3 class='timelineleft-header no-border'><a>● Order</a> <span class='text-muted'>Item has been activated by $name</span></h3>
         //            </div>
         //        </li>";
         //    }
         //    //edited
         //    else if($status == 'Edited from thriftops')
         //    {
         //        echo "<li>
         //            <i class='fa fa-circle '></i>
         //            <div class='timelineleft-item'>
         //                <span class='time'><i class='fa fa-clock-o text-danger'>&nbsp;</i>$datetime</span>
         //                <h3 class='timelineleft-header no-border'><span class='text-muted'>Item has been edited by $name</span></h3>
         //            </div>
         //        </li>";
         //    }  
         //    // display rack and warehouse where stored
         ...
         $result_order_logs= mysqli_query($mysql,"Select * from `Logs` Where Type ='Order' AND Reference='$id'");
         $row_cnt = mysqli_num_rows($result_order_logs);
         // squ in logs' reference
         while($row_logs =mysqli_fetch_assoc($result_order_logs))
         {
            $log_status = $row_logs['Status'];
            $log_orderid = $row_logs['Type_ID'];
            $log_datetime = $row_logs['DateTime'];
            $log_status == 'RReceived'? $log_status = 'received':  $log_status;
            $log_status == 'Received'? $log_status = 'received':  $log_status;
            $log_status == 'Confirmed'? $log_status = 'confirmed':  $log_status;
            $log_status == 'Booked'? $log_status = 'booked':  $log_status;
            $log_status == 'Update'? $log_status = 'updated':  $log_status;
            $log_status == 'Cancel'? $log_status = 'cancelled':  $log_status;

            echo 
            "<li>
               <i class='fa fa-circle '></i>
               <div class='timelineleft-item'>
                  <span class='time'><i class='fa fa-clock-o text-danger'>&nbsp;</i>$log_datetime</span>
                  <h3 class='timelineleft-header no-border'><span class='text-muted'>Item with Order ID: $log_orderid has been $log_status</span></h3>
               </div>
            </li>";
         }
        
         // if not in logs ( ie old orders)
         if ($row_cnt == 0){
            // echo "<h2> HEELO </h2>";
            $result_orders= mysqli_query($mysql,"Select * from `Order` Where SKU='$id'");
            while($row_ =mysqli_fetch_array($result_orders))
            {
               $orderid = $row_['Order_ID'];
               $result_id_logs = mysqli_query($mysql,"Select * from `Logs` Where Type ='Order' AND Type_ID='$orderid'");
               while($row_id_logs =mysqli_fetch_assoc($result_id_logs))
               {
                  $log_status = $row_id_logs['Status'];
                  $log_orderid = $row_id_logs['Type_ID'];
                  $log_datetime = $row_id_logs['DateTime'];
                  $log_status == 'RReceived'? $log_status = 'received':  $log_status;
                  $log_status == 'Received'? $log_status = 'received':  $log_status;
                  $log_status == 'Confirmed'? $log_status = 'confirmed':  $log_status;
                  $log_status == 'Booked'? $log_status = 'booked':  $log_status;
