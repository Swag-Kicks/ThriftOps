...
if(isset($_POST['cond']))
{
    ...
    if($cond=="all")
    {
        if(!empty($_POST['from']))
        {
             if(!empty($_POST['to']))
             {
                 $query .= ' WHERE Shipping_Status in ("Exchange","Returned","Reattempt") AND Date Between "'.$_POST['from'].'" AND "'.$_POST['to'].'"';
             }
        }
        else
        {
            $query .= ' WHERE Shipping_Status in ("Exchange","Returned","Reattempt")';
        }
    }
    if($cond=="Exchange")
    {
        if(!empty($_POST['from']))
        {
             if(!empty($_POST['to']))
             {
                 $query .= ' WHERE Shipping_Status in ("Exchange") AND Date Between "'.$_POST['from'].'" AND "'.$_POST['to'].'"';
             }
        }
        else
        {
            $query .= ' WHERE Shipping_Status in ("Exchange")';
        }
    }
       ...
       if(!empty($_POST['from']))
        {
             if(!empty($_POST['to']))
             {
                 $query .= ' WHERE Shipping_Status LIKE "%Reattempt%" AND Date Between "'.$_POST['from'].'" AND "'.$_POST['to'].'"';
             }
        }
        else
        {
            $query .= ' WHERE Shipping_Status LIKE "%Reattempt%"';
        }
    }
    if($cond=="Returned")
    {
       if(!empty($_POST['from']))
        {
             if(!empty($_POST['to']))
             {
                 $query .= ' WHERE Shipping_Status LIKE "%Returned%" AND Date Between "'.$_POST['from'].'" AND "'.$_POST['to'].'"';
             }
        }
        else
        {
            $query .= ' WHERE Shipping_Status LIKE "%Returned%"';
        }
    }
...
if(!empty($_POST['ordernum']))
{
    ...
    if(is_numeric($cut))
    {
        $query .= ' WHERE Order_Number LIKE "%#'.str_replace(' ', '%', $_POST['ordernum']).'%" AND Status in ("Exchange","Returned","Reattempt")';
    }
    else
    {
       $query .= ' WHERE Order_Number LIKE "%'.str_replace(' ', '%', $_POST['ordernum']).'%" AND Status in  ("Exchange","Returned","Reattempt")';
...
if($_POST['items'] != '')
{
  $query .= ' WHERE SKU LIKE "%'.str_replace(' ', '%', $_POST['items']).'%" AND Status in  ("Exchange","Returned","Reattempt")';
}
