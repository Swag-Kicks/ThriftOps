...
if(isset($_POST['location']))
{
    ...
    $query = "SELECT Order_Number,Order_ID,SKU FROM `Order`";
    $count=0;
    ...
    if(isset($_POST['cond']))
    {
        ...
        if($cond=="all")
        {
            if(!empty($_POST['from']))
            {
                 if(!empty($_POST['to']))
                 {
                    $query .= ' WHERE Status="Cancel" AND Inventory_Status in ("Pending","Recieved","Send Back","Not Recieved") AND '.$rt.' AND DateTime Between "'.$_POST['from'].' 00:00:00" AND "'.$_POST['to'].' 23:59:59"';
                 }
            }
            else
            {
                $query .= ' WHERE Status="Cancel" AND Inventory_Status in ("Pending","Recieved","Send Back","Not Recieved") AND '.$rt;
            }
        ...
        if($cond=="Pending")
        {
            if(!empty($_POST['from']))
            {
                 if(!empty($_POST['to']))
                 {
                     $query .= ' WHERE Status="Cancel" AND Inventory_Status LIKE "%Pending%" AND '.$rt.' AND DateTime Between "'.$_POST['from'].' 00:00:00" AND "'.$_POST['to'].' 23:59:59"';
                 }
            }
            else
            {
                $query .= ' WHERE Status="Cancel" AND Inventory_Status LIKE "%Pending%" AND '.$rt;
            }
        }
        if($cond=="Recieved")
        {
            if(!empty($_POST['from']))
            {
                 if(!empty($_POST['to']))
                 {
                     $query .= ' WHERE Status="Cancel" AND Inventory_Status LIKE "%Recieved%" AND '.$rt.' AND DateTime Between "'.$_POST['from'].' 00:00:00" AND "'.$_POST['to'].' 23:59:59"';
                 }
            }
            else
            {
                $query .= ' WHERE Status="Cancel" AND Inventory_Status LIKE "%Recieved%" AND '.$rt;
            }
        }
        if($cond=="Not Recieved")
        {
            if(!empty($_POST['from']))
            {
                 if(!empty($_POST['to']))
                 {
                     $query .= ' WHERE Status="Cancel" AND Inventory_Status LIKE "%Not Recieved%" AND '.$rt.' AND DateTime Between "'.$_POST['from'].' 00:00:00" AND "'.$_POST['to'].' 23:59:59"';
                 }
            }
            else
            {
                $query .= ' WHERE Status="Cancel" AND Inventory_Status LIKE "%Not Recieved%" AND '.$rt;
            }
        }
        if($cond=="Send Back")
        {
            if(!empty($_POST['from']))
            {
                 if(!empty($_POST['to']))
                 {
                     $query .= ' WHERE Status="Cancel" AND Inventory_Status LIKE "%Send Back%" AND '.$rt.' AND DateTime Between "'.$_POST['from'].' 00:00:00" AND "'.$_POST['to'].' 23:59:59"';
                 }
            }
            else
            {
                $query .= ' WHERE Status="Cancel" AND Inventory_Status LIKE "%Send Back%" AND '.$rt;
            }
        }
...
if($_POST['items'] != '')
{
  $query .= ' WHERE Status="Cancel" AND SKU LIKE "%'.str_replace(' ', '%', $_POST['items']).'%" AND '.$rt.' AND Inventory_Status in ("Pending","Marked")';
}
...
if($_POST['order'] != '')
{
    ...
    {
        $query .= ' WHERE Status="Cancel" AND Order_Number LIKE "%'.str_replace(' ', '%', $_POST['order']).'%" AND '.$rt.' AND Inventory_Status in ("Pending","Recieved","Send Back","Not Recieved")';
    }
    else
    {
        $query .= ' WHERE Status="Cancel" AND Order_Number LIKE "%#'.str_replace(' ', '%', $_POST['order']).'%" AND '.$rt.' AND Inventory_Status in ("Pending","Recieved","Send Back","Not Recieved")';
